...
def predict(*, inputs: Inputs) -> Outputs:
    ...
    for image, name in zip((z_adc, adc_ss), ("z_adc", "adc_ss")):
    ...
    config_filename = "/model/config_zadc_v2.json"
    with open(config_filename, "r") as op:
        config = json.load(op)
    remove_keys(config)
    config["test_filenames"] = [{"image": image_filenames[0]}]  # just use the zadc image

    prediction_images = list()
    model_filenames = sorted(glob.glob(os.path.join(config_filename.split(".json")[0], "model*.pth")))
    ...
    prediction_image = prediction_images[0].__class__(dataobj=mean_data[1], affine=prediction_images[0].affine)
    ...
    # binary_image = SimpleITK.BinaryThreshold(image1=prediction, lowerThreshold=0.5, insideValue=1, outsideValue=0)

    outputs = Outputs(hypoxic_ischemic_encephalopathy_lesion_segmentation=prediction)
    return outputs
